 ...
 * Used on both the ticker detail page (full panel) and watchlist (compact badge).
 */
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm" style={{ color: hasCritical ? "#b91c1c" : "#c2410c", opacity: 0.8 }}>
              {events.length} material event{events.length !== 1 ? "s" : ""} detected in the last 72 hours
            </p>
          </div>
        </div>
        {hasCritical && (
          <span
            className="px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider animate-pulse"
              ...
              <div className="flex items-start justify-between gap-4 flex-wrap">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-3 mt-2 text-xs text-gray-400">
                    {evt.source_name && <span>{evt.source_name}</span>}
                    {evt.published_at && (
