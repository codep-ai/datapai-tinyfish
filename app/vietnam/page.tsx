...
import { getAlertSummaryMap, getRecentRuns, getScannedTickerSet, getLatestPricesForWatchlist, countStockDirectory } from "@/lib/db";
...
export default async function VietnamPage() {
  ...
  const vnAlertCount = VN_UNIVERSE.filter((tk) => !!alertMap[tk.symbol]).length;
  ...
  return (
    <div>
      ...
      <div
        ...
        style={{
          background: "linear-gradient(135deg, #c8102e 0%, #da291c 40%, #ff6b35 100%)",
          paddingTop: "28px",
          paddingBottom: "28px",
        }}
      >
        <div className="max-w-6xl mx-auto px-6 space-y-3">
          ...
          <div className="flex gap-3 items-center flex-wrap">
            <Link
              ...
              style={{ fontSize: "0.9rem", background: "#ffd700", color: "#c8102e" }}
            >
      ...
      <div className="max-w-6xl mx-auto px-6 py-10 space-y-12">
        ...
        <div>
          <div className="flex items-center justify-between mb-5">
            ...
            <div className="flex items-center gap-4 text-sm text-gray-400">
              <Link href="/alerts" className="hover:underline" style={{ color: "#c8102e" }}>{t(labels, "hero_view_alerts")} →</Link>
            </div>
          </div>

          {(() => {
            const sorted = [...VN_UNIVERSE].sort((a, b) => {
                ...
                {sorted.map((tk) => {
                  ...
                  return (
                    <div key={tk.symbol} className="relative rounded-xl px-4 pt-6 pb-4 transition-all duration-200 group shadow-sm hover:-translate-y-0.5" style={cardStyle}>
                      {hasAlert && (
                        <span className="absolute -top-2.5 -right-2.5 text-xs font-bold px-2 py-0.5 rounded-full shadow" style={{ background: "#c8102e", color: "#fff" }}>
                      ...
                      <Link href={`/ticker/${tk.symbol}`} className="block pr-6">
                        <div className="font-bold text-base group-hover:opacity-80" style={{ color: "#c8102e" }}>{tk.symbol}</div>
                      ...
                      return (
                        <tr key={tk.symbol} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors" style={{ background: rowBg }}>
                          ...
                          <td className="px-4 py-3">
                            <Link href={`/ticker/${tk.symbol}`} className="font-bold hover:opacity-80" style={{ color: "#c8102e" }}>{tk.symbol}</Link>
                          </td>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            {[
              ...
              <div key={item.step} className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
                <div className="font-bold text-3xl mb-3" style={{ color: "#c8102e" }}>{item.step}</div>
        ...
        <div className="border border-red-200 rounded-2xl p-8 bg-white shadow-sm">
          <h3 className="font-bold text-xl mb-4" style={{ color: "#c8102e" }}>
