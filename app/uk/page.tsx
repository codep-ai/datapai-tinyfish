...
export default async function UkPage() {
  ...
  const stocks = await getActiveStocks("LSE", lang, 30, true);
  const [alertMap, scannedSet, recentRuns, priceMap] = await Promise.all([
    getAlertSummaryMap(),
    getScannedTickerSet(),
    getRecentRuns(3),
    getLatestPricesForWatchlist(stocks.map((s) => ({ symbol: s.symbol, exchange: "LSE" }))),
  ]);
  const lastRun = recentRuns[0] ?? null;
  const lseAlertCount = stocks.filter((s) => !!alertMap[s.symbol]).length;
      ...
      >
        <div className="max-w-6xl mx-auto px-6 space-y-3">
          ...
          <h1 className="text-2xl font-bold text-white">
            {t(labels, "hero_title_uk")}
          </h1>

          <p className="text-white/80 text-sm font-medium">
            {t(labels, "hero_spot_shifts")} —{" "}
            <span className="text-white font-bold">350+ {t(labels, "hero_stocks_covered")}</span>{" "}
                      ...
                      <Link href={`/ticker/${tk.symbol}?exchange=${tk.exchange}`} className="block pr-6">
                        ...
                        <div className="mt-2 space-y-0.5">
                          {price && closeNum !== null && !isNaN(closeNum) ? (
                            <>
                              <div className="text-sm font-semibold text-gray-700">{closeNum.toFixed(2)}p</div>
                      ...
                      return (
                        <tr key={tk.symbol} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors" style={{ background: rowBg }}>
                          ...
                          <td className="px-4 py-3 text-right font-semibold text-gray-700">
                            {closeNum !== null && !isNaN(closeNum) ? `${closeNum.toFixed(2)}p` : "\u2014"}
                          </td>
