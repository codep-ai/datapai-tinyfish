 ...
 * Targeting Japan + Asia-Pacific investors.
 */
...
export default async function JapanPage() {
  ...
  const stocks = await getActiveStocks("TSE", lang, 30, true);
  const [labels, alertMap, scannedSet, recentRuns, totalTse, priceMap] = await Promise.all([
    loadTranslations(lang),
    getAlertSummaryMap(),
    getScannedTickerSet(),
    getRecentRuns(3),
    countActiveStocks("TSE"),
    getLatestPricesForWatchlist(stocks.map((s) => ({ symbol: s.symbol, exchange: "TSE" }))),
  ]);
      ...
      >
        <div className="max-w-6xl mx-auto px-6 space-y-3">
          <h1 className="text-2xl font-bold text-white">
            {t(labels, "hero_title_tse")}
          </h1>

          <p className="text-white/80 text-sm font-medium">
            {t(labels, "section_tse_label")} —{" "}
            <span className="text-white font-bold">{totalTse.toLocaleString()} {t(labels, "hero_stocks_covered")}</span>{" "}
                      ...
                      <Link href={`/ticker/${tk.symbol}?exchange=TSE`} className="block pr-6">
                        ...
                        <div className="mt-2 space-y-0.5">
                          {closeNum !== null && !isNaN(closeNum) ? (
                            <>
                              <div className="text-sm font-semibold text-gray-700">{"\u00A5"}{fmtJPY(closeNum)}</div>
                      ...
                      return (
                        <tr key={tk.symbol} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors" style={{ background: rowBg }}>
                          ...
                          <td className="px-4 py-3 text-right font-semibold text-gray-700">
                            {closeNum !== null && !isNaN(closeNum) ? `\u00A5${fmtJPY(closeNum)}` : "—"}
                          </td>
