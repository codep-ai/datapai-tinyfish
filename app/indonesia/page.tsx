/**
 * /indonesia  —  Indonesia Stocks (IDX)
 * 200+ tickers in DB, top-20 blue chips shown, rest searchable. Targeting Indonesian investors.
 */
...
export default async function IndonesiaPage() {
  ...
  const stocks = await getActiveStocks("IDX", lang, 30, true);
  const [labels, alertMap, scannedSet, recentRuns, priceMap, totalId] = await Promise.all([
    loadTranslations(lang),
    getAlertSummaryMap(),
    getScannedTickerSet(),
    getRecentRuns(3),
    getLatestPricesForWatchlist(stocks.map((s) => ({ symbol: s.symbol, exchange: "IDX" }))),
    countActiveStocks("IDX"),
  ]);
  const lastRun = recentRuns[0] ?? null;
  const idAlertCount = stocks.filter((s) => !!alertMap[s.symbol]).length;
      ...
      >
        <div className="max-w-6xl mx-auto px-6 space-y-3">
          <h1 className="text-2xl font-bold text-white">
            {t(labels, "hero_title_idx")}
          </h1>

          <p className="text-white/80 text-sm font-medium">
            {t(labels, "section_idx_market")} —{" "}
            <span className="text-white font-bold">{totalId.toLocaleString()} {t(labels, "hero_stocks_covered")}</span>{" "}
      ...
      <div className="max-w-6xl mx-auto px-6 py-10 space-y-12">
        ...
        {lastRun && (
          <div className="flex items-center gap-4 text-sm text-gray-500 border border-gray-100 rounded-lg px-4 py-3 bg-white shadow-sm">
            ...
            <span className="text-gray-300">·</span>
            <span style={{ color: "#2e8b57" }}><strong>{idAlertCount}</strong> {t(labels, "tickers_with_data")}</span>
        ...
        <div>
          <div className="flex items-center justify-between mb-5">
            <h2 className="text-3xl font-bold text-[#252525]">
              ...
              <span className="text-lg font-normal text-gray-400 ml-2">{t(labels, "section_idx_market")}</span>
            </h2>
                  ...
                  return (
                    <div key={tk.symbol} className="relative rounded-xl px-4 pt-6 pb-4 transition-all duration-200 group shadow-sm hover:-translate-y-0.5" style={cardStyle}>
                      ...
                      <div className="absolute top-1.5 right-1.5 z-10">
                        <WatchlistButton compact symbol={tk.symbol} exchange="IDX" name={tk.name} />
                      </div>
                      <Link href={`/ticker/${tk.symbol}?exchange=${tk.exchange}`} className="block pr-6">
                        ...
                        <div className="mt-2 space-y-0.5">
                          {price && closeNum !== null && !isNaN(closeNum) ? (
                            <>
                              <div className="text-sm font-semibold text-gray-700">Rp{fmtIDR(closeNum)}</div>
                      ...
                      return (
                        <tr key={tk.symbol} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors" style={{ background: rowBg }}>
                          ...
                          <td className="px-4 py-3 text-right font-semibold text-gray-700">
                            {closeNum !== null && !isNaN(closeNum) ? `Rp${fmtIDR(closeNum)}` : "—"}
                          </td>
        ...
        <div className="border border-red-200 rounded-2xl p-8 bg-white shadow-sm">
          <h3 className="font-bold text-xl mb-4" style={{ color: "#2e8b57" }}>
            🇮🇩 {t(labels, "section_idx_market")} — {t(labels, "trust_title")}
          </h3>
